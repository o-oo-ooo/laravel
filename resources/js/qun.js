...
function quitQun(qid)
{
	...
	MessageBox('confirm', "你确定退出吗？", '提示', param);
}
...
function goQun(qid){
	if(qid < 1){
		show_message('请确定你要进入的'+__WEIQUN__);
		return false;
	}
...
function showRecommendQunDialog(qid)
{
	...
	showDialog(handle_key, 'ajax', '推荐', {url:'ajax.php?mod=qun&code=recd2w&qid='+qid}, 500);
}
...
function dismissQun(qid)
{
	...
	MessageBox('confirm', "你确定要解散当前"+__WEIQUN__+"吗？", '提示', param);
}
