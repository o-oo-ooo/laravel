...
function del_item(obj,oid,type)
{
	...
	var o = obj.parentNode.parentNode;
	//增加条件判断，因火狐和Chrome均对空白算一个节点，这个BUG有点小讨厌
...
function add_item(obj,item,itemid,type)
{
	...
	if(nickname.length == 0){
		...
		if(d == -1){
			...
			c3.innerHTML = '<a href="javascript:;" onclick="del_item(this,0);return false;" class="icon-del">删除</a>';
			user[ui] = d;oj.value = '';
			ui++;
		}else{
			MessageBox('notice', "未知错误！", '提示');
		}
		});
	}
	oi.focus();oi.value = '';
